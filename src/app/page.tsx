      ...
      <section className="py-16 bg-background relative border-t border-b border-foreground/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center space-y-3 mb-12">
            <h2 className="text-2xl font-bold sm:text-3xl tracking-tight text-foreground capitalize">Interactive Health Simulator</h2>
            <p className="text-sm text-foreground/60 max-w-2xl mx-auto font-medium">
              Drag the sliders below to see how stress, sleep, and work hours impact your recovery and long-term health in real-time.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            ...
            <div className="lg:col-span-5 flex flex-col justify-between p-6 rounded-2xl glass-panel border-foreground/5 bg-foreground/[0.02]">
              <div className="space-y-6">
                <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
                  <Compass className="h-4.5 w-4.5 text-primary animate-spin-slow" />
                  Habit & Lifestyle Adjuster
                </h3>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-foreground/50">Higher stress drains mental energy and slows muscle recovery.</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-foreground/50">Longer, consistent sleep triggers optimal mental and physical recovery.</p>
                </div>

                {/* Workload hours slider */}
                <div className="space-y-2">
                  <div className="flex justify-between text-xs font-bold">
                    <span>Daily Work / Coding Hours</span>
                  ...
                  />
                  <p className="text-xs text-foreground/50">Extended focus work increases fatigue and requires longer rest periods.</p>
                </div>
              </div>

              <div className="border-t border-foreground/5 pt-4 mt-6 text-xs text-foreground/50 font-medium">
                Note: This interactive console demonstrates how VitalCore AI maps your habits to predict burnout.
              </div>
            </div>
            ...
            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              {/* Recovery card */}
              <GlassCard glowColor={simRecovery > 65 ? "emerald" : "rose"} className="flex flex-col justify-between p-5 rounded-2xl">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-[var(--muted)]">Recovery Score</span>
                  <HeartPulse className="h-4.5 w-4.5 text-secondary" />
                </div>
                <div className="my-3">
                  <span className="analytics-number text-[var(--foreground)] text-3xl font-extrabold">{simRecovery}%</span>
                </div>
                <div className="text-xs text-[var(--muted)] leading-normal font-medium">
                  {simRecovery > 65 ? "Your body is primed for optimal performance." : "Sleep debt detected. Focus on rest and hydration today."}
                </div>
              </GlassCard>

              {/* Burnout risk card */}
              <GlassCard glowColor={simBurnoutRisk > 60 ? "rose" : "violet"} className="flex flex-col justify-between p-5 rounded-2xl">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-[var(--muted)]">Burnout Risk</span>
                  <AlertTriangle className="h-4.5 w-4.5 text-accent" />
                </div>
                <div className="my-3">
                  <span className="analytics-number text-[var(--foreground)] text-3xl font-extrabold">{simBurnoutRisk}%</span>
                </div>
                <div className="text-xs text-[var(--muted)] leading-normal font-medium">
                  {simBurnoutRisk > 60 ? "High burnout risk. Consider planning a light rest day." : "Your health parameters are perfectly balanced."}
                </div>
              </GlassCard>

              {/* Wellness stability score */}
              <GlassCard glowColor={simStability > 70 ? "violet" : "amber"} className="flex flex-col justify-between p-5 rounded-2xl">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-[var(--muted)]">Health Stability</span>
                  <Activity className="h-4.5 w-4.5 text-primary" />
                </div>
                <div className="my-3">
                  <span className="analytics-number text-[var(--foreground)] text-3xl font-extrabold">{simStability}%</span>
                </div>
                <div className="text-xs text-[var(--muted)] leading-normal font-medium">
                  {simStability > 70 ? "Great routine balance! Keep doing what you are doing." : "System strain detected. Prioritize early sleep tonight."}
                </div>
              </GlassCard>

              {/* digital twin future version card span */}
              <div className="sm:col-span-3 rounded-2xl glass-panel p-5 border-foreground/5 flex flex-col sm:flex-row items-center justify-between gap-4 bg-foreground/[0.01]">
