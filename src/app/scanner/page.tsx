          ...
          <div className="lg:col-span-5 rounded-2xl glass-panel p-6 border-foreground/5 flex flex-col justify-between space-y-6">
            <div className="space-y-6">
              ...
              >
                ...
                <span className="text-[10px] text-foreground/50">Supports JPEG, PNG biometrics</span>
              </div>

              {/* Barcode scanner form */}
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-foreground">Barcode / Food Item Search</label>
              ...
              <div className="space-y-2 pt-2">
                <span className="text-[9px] font-bold text-foreground/50 uppercase block">Instant Demos</span>
                <div className="flex flex-wrap gap-2">
                  {sampleFoods.map((s, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleScanSimulation(s.query)}
                      className="px-2.5 py-1.5 text-[10px] font-bold rounded-lg bg-foreground/5 border border-foreground/5 hover:border-primary/30 transition-all cursor-pointer text-foreground/75"
                    >
                      {s.label}
                    </button>
                  ))}
          ...
          <div className="lg:col-span-7 rounded-2xl glass-panel p-6 border-foreground/5 flex flex-col justify-center min-h-[380px]">
            ...
            {scanning && (
              <div className="flex flex-col items-center gap-3 text-center py-10">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-primary animate-pulse">Running Neural Scanner...</span>
                <p className="text-[10px] text-foreground/60 max-w-xs">Extracting glycemic vectors, calorie estimates, and packaging chemicals logs.</p>
              </div>
            )}
            ...
            {!scanning && !result && (
              <div className="flex flex-col items-center gap-2 text-center py-10 text-foreground/45 select-none">
                ...
                <span className="text-xs font-extrabold uppercase">Console Empty</span>
                <p className="text-[10px] max-w-xs">Upload a food snapshot or scan a barcode to generate macromolecule reports instantly.</p>
              </div>
            )}
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="space-y-0.5">
                    ...
                    <h3 className="text-sm font-extrabold text-foreground mt-1">{result.foodName}</h3>
                  </div>
                  ...
                  <div className={`h-14 w-14 rounded-full border-2 flex flex-col items-center justify-center shrink-0 shadow-lg ${
                    ...
                    <span className="text-base font-extrabold">{result.healthScore}</span>
                  </div>
                </div>

                {/* Macro breakdown grid */}
                <div className="grid grid-cols-4 gap-3 text-center text-[10px] font-extrabold">
                  <div className="bg-foreground/5 p-2 rounded-lg border border-foreground/5">
                    ...
                    <span className="text-sm font-extrabold block mt-0.5">{result.protein}g</span>
                  </div>

                  <div className="bg-foreground/5 p-2 rounded-lg border border-foreground/5">
                    <span className="text-secondary block font-bold">Carbs</span>
                    <span className="text-sm font-extrabold block mt-0.5">{result.carbs}g</span>
                  </div>

                  <div className="bg-foreground/5 p-2 rounded-lg border border-foreground/5">
                    <span className="text-accent block font-bold">Fats</span>
                    <span className="text-sm font-extrabold block mt-0.5">{result.fat}g</span>
                  </div>

                  <div className="bg-foreground/5 p-2 rounded-lg border border-foreground/5">
                    <span className="text-red-400 block font-bold">Sugars</span>
                    <span className="text-sm font-extrabold block mt-0.5">{result.sugar}g</span>
                  </div>
                </div>

                {/* Sugar Warning */}
                {result.sugarAlert && (
                  <div className="rounded-xl border border-red-500/20 bg-red-500/5 px-3 py-2.5 text-[10px] text-red-500 font-semibold flex items-center gap-2">
                ...
                {result.unhealthyAdditives.length > 0 && (
                  <div className="space-y-1">
                    <span className="text-[9px] font-bold text-red-500 uppercase block">Unhealthy Chemical Additives Detected:</span>
                    <div className="flex flex-wrap gap-2">
                ...
                <div className="space-y-1">
                  <span className="text-[9px] font-bold text-foreground/50 uppercase block">Preventative Diagnostics:</span>
                ...
                <div className="space-y-2 pt-2 border-t border-foreground/5">
                  <span className="text-[9px] font-bold text-secondary uppercase block">Healthier Alternative Suggestions:</span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px] font-bold text-foreground/80">
